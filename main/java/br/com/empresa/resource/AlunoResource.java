...
@RestController
@RequestMapping("/aluno") //incluir no endereço
public class AlunoResource {

	@Autowired
	private AlunoService alunoService;
	
	@RequestMapping(method=RequestMethod.GET) //casoa requisição for do tipo get, retornará a lista de aluno.
	public ResponseEntity<List<Aluno>> listarAluno(){
		List<Aluno> alunos = alunoService.listaTodosAlunos();
		return ResponseEntity.ok().body(alunos); //se for ok irá colocar no corpo a lista de alunos.
	}
